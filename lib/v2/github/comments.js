...
module.exports = async (ctx) => {
    ...
    if (isNaN(number)) {
        const response = await got(`${apiUrl}/repos/${user}/${repo}/issues/comments`, { headers, searchParams: { sort: 'updated', direction: 'desc', per_page: limit, }, });

    if (!response.data) {
        throw new Error('Failed to fetch the issues and comments');
    }
...
async function singleIssue(ctx, user, repo, number, limit, headers) {
    const response = await got(`${apiUrl}/repos/${user}/${repo}/issues/${number}`, {headers}, {
        headers,
    });
